...
export const Authenticate = async (uid: string, name: string, fire_token: string, callback: (error: string | null, user: UserType | null) => void) => {
  try {
...
export const Validate = async (fire_token: string, callback: (error: string | null, user: UserType | null) => void) => {
  try {
    const response = await axios({
      method: 'GET',
...
export const SignInWithSocialMedia = (provider: firebase.auth.AuthProvider) => new Promise<firebase.auth.UserCredential>((resolve, reject) => {
